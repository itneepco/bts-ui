    ...
    return Controller.extend("bts.controller.Display_Report", {
      ...
      _onRouteMatched: function (oEvent) {
        ...
        function checkvalidation() {
          ...
          var Approve = that.getView().byId("Approve");
          var cancel = that.getView().byId("Cancel");
          var Reject = that.getView().byId("Reject");
          var Re_Reject = that.getView().byId("Re_Reject");
          var Re_Approve = that.getView().byId("Re_Approve");

          if (status1 === "PENDING") {
            //if status is pending then all the fields can be editable (For Eg 0001 is pending)
            Approve.setVisible(true);
            Reject.setVisible(true);
            Re_Reject.setVisible(false);
            Re_Approve.setVisible(false);
          } else if (status1 === "APPROVED") {
            Re_Approve.setVisible(true);
          } else if (status1 === "REJECTED") {
            Re_Reject.setVisible(true);
          } else {
      ...
      OnReject: function () {
        ...
        function openRejectRemarksPopup() {
          ...
          if (!oReasonDialog) {
            ...
            var radioId = "reasonRadio_" + Date.now();

            // Create a dialog for the reason input
            var oReasonDialog = new sap.m.Dialog({
              title: "Reason for Rejection",
              content: [
                // new sap.m.Label({ text: "Select a reason:" }),
                new sap.m.RadioButtonGroup(radioId, {
                  columns: 1,
                  selectedIndex: 0, // -1 nothing selected initially
                  buttons: [
                    new sap.m.RadioButton({ text: "Invalid Vendor Details" }),
                    new sap.m.RadioButton({ text: "Mismatch in PO and GRN/SES" }),
                    new sap.m.RadioButton({ text: "Invalid Supporting Documents" }),
                    new sap.m.RadioButton({ text: "Amount Discrepancy" }),
                    new sap.m.RadioButton({ text: "Incomplete Documents" }),
                    new sap.m.RadioButton({ text: "Duplicate or Already Processed Entry" })
                  ]
                }),
                // new sap.m.Label({ text: "Explanation (optional):" }),
                new sap.m.TextArea({
                  id: popid,
                  placeholder: "Provide additional reason for rejection...",
                  width: "100%",
                  rows: 4,
                  maxLength: 500,
                  growing: false,
              ...
              buttons: [
                ...
                new sap.m.Button({
                  text: "Submit",
                  press: function () {
                    var oRadioGroup = sap.ui.getCore().byId(radioId);
                    var selectedIndex = oRadioGroup.getSelectedIndex();
                    var selectedReason = "";
                    if (selectedIndex !== -1) {
                      selectedReason = oRadioGroup.getButtons()[selectedIndex].getText();
                    }

                    var explanation = sap.ui.getCore().byId(popid).getValue();

                    if (!selectedReason) {
                      sap.m.MessageToast.show("Please select a reason for rejection.");
                      return;
                    } else if (!explanation) {
                      sap.m.MessageToast.show("Please give a additional detailed reason for rejection.");
                      return;
                    }

                    // Combine reason + explanation
                    ...
                    var jsonData = {
                      ...
                      REJECTED_TEXT: explanation
                    };
                    console.log("JSON DATA: ", jsonData);

                    oModel.update(sEntitySet, jsonData, {
                      success: function (Odata) {
                        console.log("Data Rejected successfully:", Odata);
                        sap.m.MessageBox.success(
                          "BTS ID " + BTSValue + " is Rejected successfully",
                          {
                            title: "Success",
                            onClose: function (sAction) {
                              location.reload();
      ...
      OnReReject: function () {
        ...
        function openAccept() {
          ...
          oModel.update(sEntitySet, jsonData, {
            success: function (Odata) {
              console.log("Data Accepted successfully:", Odata);
              sap.m.MessageBox.success(
                "BTS ID " + BTSValue + " is Revoke from Rejected successfully",
                {
                  title: "Success",
                  onClose: function (sAction) {
                    location.reload();
        ...
        function openAccept() {
          ...
          oModel.update(sEntitySet, jsonData, {
            success: function (Odata) {
              console.log("Data Accepted successfully:", Odata);
              sap.m.MessageBox.success(
                "BTS ID " + BTSValue + " is Accepted successfully",
                {
                  title: "Success",
                  onClose: function (sAction) {
                    location.reload();
        ...
        function openAccept() {
          ...
          oModel.update(sEntitySet, jsonData, {
            success: function (Odata) {
              console.log("Data Accepted successfully:", Odata);
              sap.m.MessageBox.success(
                "BTS ID " + BTSValue + " is Revoke from Accepted successfully",
                {
                  title: "Success",
                  onClose: function (sAction) {
                    location.reload();
